...
public class MethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private void validateValue(Object value, String cName, Validator[] validators, MethodParameter parameter)
        ...
        for (Validator validator1 : validators) {
            Validator validator = Validator.getValidator(validator1);
            if (validator != null) {
                ...
                LOGGER.error("验证器[" + validator1 + "],在Validator.java文件中没有定义，请检查！");
            }
        }
    }
    ...
    private ParamInfo getParamInfo(MethodParameter parameter) {
        return this.paramInfoCache.computeIfAbsent(parameter, k -> createParamInfo(parameter));
    }
